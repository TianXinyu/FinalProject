...
## use boxplot to show the average age of both lbfc statues in PR
```{r}
boxplot(prdata$AGE~prdata$lbfc)
```

## I sepret age into Y (young25-45) and O(old,45-65)
```{r}
...
## compare the difference between young people
```{r}
prop.table(table(prY$lbfc))
prop.table(table(usY$lbfc))
```

## compare the difference between old people
```{r}
prop.table(table(prO$lbfc))
prop.table(table(usO$lbfc))
```

# use probiblity table to see the disability rate in PR and US
```{r}
prop.table(table(prdata$Disable))
prop.table(table(usdata$Disable))
```

## use boxplot to show the average age of both health statues in PR
```{r}
boxplot(prdata$AGE~prdata$Disable)
```

## For people who are disabled, how many of them are in labor force
```{r}
